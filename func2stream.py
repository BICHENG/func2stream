...
class Pipeline(Element):
    def __init__(self, elements: list, friendly_name="Pipeline"):
        ...
        print(f"Building\t┌{self.friendly_name} connections, total {len(self.elements)} elements：┐")
        
        for i in range(len(self.elements) - 1):
            self.elements[i]._link_to(self.elements[i + 1])
            print(f"\t│Link {self.elements[i].friendly_name} -> {self.elements[i + 1].friendly_name} has been established [{i+1}/{len(self.elements)-1}]")
        print("\t└───────────────────")
        for i, elm in enumerate(self.elements):
    ...
    def start(self):
        ...
        if self.elements[-1].sink is None:
            ...
            print(f"SINK {self.elements[-1].friendly_name} will be a queue that automatically discards data")
